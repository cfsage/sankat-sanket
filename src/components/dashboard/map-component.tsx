...
const createIcon = (type: 'incident' | 'pledge', severity?: 'Low' | 'Medium' | 'High') => {
  const isHighSeverity = type === 'incident' && severity === 'High';

  const pulseAnimation = isHighSeverity ? 'animate-pulse' : '';

  const iconMarkup = renderToStaticMarkup(
    <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white border-4 shadow-lg ${pulseAnimation} 
                    ...
                    <Popup maxWidth={350}>
                         <Card className="border-0 shadow-none">
                            ...
                            <CardContent>
                                ...
                                {incident.needs.map(need => <Badge key={need} variant="outline">{need}</Badge>)}
                                </div>
                            </CardContent>
                        </Card>
                    </Popup>
